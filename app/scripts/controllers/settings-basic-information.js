...
angular.module('webAdminApp')
  .controller('BasicInformationCtrl', function($scope, $rootScope, $state, siteApi, storage, toastr, toastrConfig) {
    ...
    $scope.saveSiteInfo = function(site, business) {
      siteApi.update($scope.authToken, site).then(function(data) {
        console.log(business);
        $scope.saveBusinessInfo(site.id, business);
      }, function(response) {
        var message = 'Something bad happened :(';
        if ((response.status === 401 || response.status === 422) && response.data && response.data.error) {
          message = response.data.error.message;
    ...
    $scope.saveBusinessInfo = function(siteId, business) {
      if ($scope.business.id === null) {
        siteApi.createBusiness($scope.authToken, siteId, business).then(function(data) {
    ...
    $scope.saveBasicInfo = function() {
      ...
      $scope.saveSiteInfo($scope.site, $scope.business);
    };
  });
